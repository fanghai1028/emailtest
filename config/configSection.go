...
type TailConfigEntity struct {
	FileName            string         // 文件名
	FileNameUseTemplate bool           // 文件名是否使用模版 
...
type SmtpMailServerEntity struct {
	ServerAddress     string       // 邮件服务器地址，不包含端口号43.138.49.35
	ServerAddressPort int          // 邮件服务器端口号，默认 25
	NeedLogin         bool         // 邮箱需要登陆么？ true 需要登陆， false 不用
	LoginUser         string       // 邮箱登陆名   fanghai@zcy-inc.club
	LoginPassword     string       // 邮箱登陆密码   Fanghai@123
